...
RUN echo "hwloc_base_binding_policy = none" >> /usr/local/etc/openmpi-mca-params.conf && \
    ...
    echo "btl_tcp_if_include = ib0" >> /usr/local/etc/openmpi-mca-params.conf

# Set default NCCL parameters
RUN echo NCCL_DEBUG=INFO >> /etc/nccl.conf && \
    echo NCCL_SOCKET_IFNAME=ib0 >> /etc/nccl.conf
...
ARG SSH_PORT_NO=8443

RUN mkdir /root/.ssh
RUN pwd

COPY hostfile.txt /examples
COPY .ssh/* /root/.ssh/
